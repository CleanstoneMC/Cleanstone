<template>
    <section class="section" :class="{ 'is-loading' : !hasLoaded }">
        ...
        <sba-panel :header-sticks-below="['#navigation']"
                   v-for="(beanDefinition, beanName) in beans" :key="beanName"
                   :title="beanName">
            <table class="table is-fullwidth">
                <tr v-for="(value, key) in beanDefinition" :key="`${key}-${value}`">
                    <td>
                        <span v-text="key"/><br>
                        <small class="is-muted" v-if="value" v-text="value"/>
                    </td>
                </tr>
            </table>
            <p class="is-muted" v-else>No Beans set</p>
        </sba-panel>
    </section>
</template>
