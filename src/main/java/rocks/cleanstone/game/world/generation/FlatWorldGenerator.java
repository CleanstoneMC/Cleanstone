...
public class FlatWorldGenerator implements WorldGenerator {
    ...
    @Autowired
    public FlatWorldGenerator(BlockStateProvider blockStateProvider, ImmutableBlockProvider immutableBlockProvider,
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                blockDataTable.setBlock(x, 45, z, immutableBlockProvider.of(VanillaBlockType.GRASS));
                for (int y = 44; y > 0; y--) {
                    blockDataTable.setBlock(x, y, z, immutableBlockProvider.of(VanillaBlockType.STONE));
                }

                blockDataTable.setBlock(x, 0, z, immutableBlockProvider.of(VanillaBlockType.BEDROCK));
